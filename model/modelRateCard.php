...
if(isset($_POST['action'])){
    ...
    if($action == "AddProducts"){
        ...
        $query = mysqli_query($con , "INSERT INTO `rate_card_prices`(`rate_card_id`, `prod_id`, `date_created`, `is_active`) VALUES ('{$rateCardId}' , '{$productId}' , '{$time}', 'True')");
        if($query){
    ...
    if($action == "RemoveProduct"){
        $rateCardId = $_POST['rateCardId'];
        $productId = $_POST['prod_id'];
        $query = mysqli_query($con , "DELETE FROM `rate_card_prices` WHERE `rate_card_id` = '{$rateCardId}' AND `prod_id` = '{$productId}'");
        if($query){
